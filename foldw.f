      ...
      DIMENSION COULOMB99(MAX_PTS),COULCC(MAX_PTS),COULFOLD(MAX_PTS)
      DIMENSION RHOCC(MAX_PTS),POTIN2(MAX_PTS),POTIN3(100),qspn(2)
     ...
     &      ZWATM(1000),RATMS(3,1000),ITSCF
      ...
      COMMON/POTW/POTINW(MAX_PTS),COULOMBW(MAX_PTS)
      DATA FIRST/.TRUE./
      DATA FIRSTLAT/.TRUE./
...
! V_ext(eN) from Wannier Domain evaluated in Central Cell.
     ...
     &           (RMSH(3,IMSH)-vz)**2
          ...
          POTINW(IMSH)= POTINW(IMSH)-ZWATM(iatm-MMSH1)*DIST !make ZATMS general
       ENDDO
       ENDDO
...
!        qtt2=0.0d0
!        do ipts=1,NMSHW
!        qttt=qttt+RHOG(ipts,1,1)*WMSH(ipts)
!        qtt2=qtt2+RHOG(ipts,1,2)*WMSH(ipts)
!        enddo
!        print *,'beforesave',qttt,qtt2
!
!
!        RHOG=0.0d0

        !call save_rhow(NMSHW)
...
!           DO IGRAD=1,NGRAD
!              RHOG(IMSH,IGRAD,ISPN)=RHOG(IMSH,IGRAD,ISPN)+
!     &            RHOG(NMSH+NMSHW*(ILAT-1)+IMSH,IGRAD,ISPN)
!             qtest9=qtest9+RHOG(NMSH+NMSHW*(ILAT-1)+IMSH,IGRAD,ISPN)
!           ENDDO
!           ENDDO
!        ENDDO 
!        ENDDO
!       print *,'remaining charge',qtest9
...
! Emperically adding missing charges
!        DO ISPN=1,NSPN
!           qspn(ispn)=0.0d0
!        DO IMSH=1,NMSHW
!           qspn(ispn)=qspn(ispn)+RHOG(IMSH,1,ISPN)*WMSH(imsh)
!        ENDDO
!        ENDDO
!        print *,'qspn up and down',qspn(1),qspn(2)
...
!        DO IGRAD=1,NGRAD
!         if(ispn.eq.1)then
!         qt10=qt10+RHOG(IMSH,1,1)*(12.0d0/qspn(1))*WMSH(imsh)
!         RHOG(IMSH,IGRAD,ISPN)=RHOG(IMSH,IGRAD,ISPN)*(12.0d0/qspn(ispn))
!         else
!         RHOG(IMSH,IGRAD,ISPN)=RHOG(IMSH,IGRAD,ISPN)*(6.0d0/qspn(ispn))
!         endif
!         ENDDO
!        ENDDO
!        ENDDO
!        print *,'Corrected charge:',qt10
!==========================================================================
...
!==========================================================================
       ...
       DO iwd=0,NDCELL-1
          ...
          POTIN3(iat)=POTIN3(iat)+pottemp
       ENDDO
       ENDDO

       ELOCALW=0.0D0
       DO j=1,NATMS
       ELOCALW=ELOCALW-0.5D0*ZWATM(j)*POTIN3(j)
       ENDDO
!==========================================================================
! ENNUCW
!==========================================================================
     ...
     &          (RMSH(3,MMSH1+iat)-vz)**2
         ...
         DIST=1.0D0/DSQRT(DIST)
         POTIN2(iat) = POTIN2(iat)+ZWATM(jat)*DIST
         ENDIF
         ENDDO
         ENDDO
         ENDDO
...
! nuclie's in WD (E_NN belong to WD).
         ENNUCW=0.0d0
         DO j=1,NATMS
         ENNUCW=ENNUCW+0.5d0*ZWATM(j)*POTIN2(j)
         ENDDO
         FIRST=.FALSE.
