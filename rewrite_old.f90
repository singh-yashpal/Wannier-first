subroutine rewrite_symbol(ncalc,mode_run,imesh,ierr)
    implicit none
    integer, parameter :: max_atoms = 400, max_types = 10
    ...
    real(8), dimension(3) :: ttvec
    real(8), dimension(100) :: alpha, coef
    integer, dimension(1000, 3) :: nt
    logical :: found, exist
    integer :: i, j, k, ii, iper, ios, icell, ic, eq_pos
    integer :: is,ip,id,iss,isp,isd
    ...
    integer :: mcalc,mode_run,imesh,ierr,ncalc
    real(8) :: x, y, z, ucut, twistang, twistax, barcut, barsht
    real(8) :: BBOUND(3,2),TVEC(3,3),TWIST(3,3,3),ROT(3,3)
    real(8) :: RSPHWD

    common /PTRANS/ BBOUND, TVEC, TWIST, ROT, RSPHWD, NXYZ
...
! Try reading one line to check if file is empty
   read(20, *, iostat=ios) nt(1,1), nt(1,2), nt(1,3)
   if (ios /= 0) then
    ...
    rewind(20)
    do i=1,10000
       read(20,*,end=1011)nt(icell,1),nt(icell,2),nt(icell,3)
       icell=icell+1
    enddo
 1011    continue
...
!    print *, "Translated atom positions for neighboring cells:"
    ...
    do i = 1, icell - 1  ! Loop over UTRANS entries
            ...
            do k=1,3
               TTVEC(k)=nt(i,1)*TVEC(k,1)+nt(i,2)*TVEC(k,2)+nt(i,3)*TVEC(k,3)
            enddo
            ...
            wcoords(1,ic,j) = x + ttvec(1)  ! tvec(:,1) is the 1D vector
            wcoords(2,ic,j) = y + ttvec(2)  ! tvec(:,1) is the 1D vector
            wcoords(3,ic,j) = z + ttvec(3)  ! tvec(:,1) is the 1D vector

!            print '(A10, 3F12.6)', trim(type_list(j)), x, y, z
            end do
    end do
      atom_counts(j)=ic
    end do
    ...
    do i=1,atom_counts(j)
       dist(j,i)=sqrt(wcoords(1,i,j)**2+wcoords(2,i,j)**2+wcoords(3,i,j)**2)
