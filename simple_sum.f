c 400 1.644934068    1.202056903    1.082323234    1.036927755 1.017343062    1.008349277    1.004077356    1.002008393    1.000994575
      implicit real*8 (a-h,o-z)
        dimension sum(10,4),alp(10)
        dimension xn(10),wn(10),zeta(10)
        pi=4.0d0*atan(1.0d0)
             zeta=0.0d0
          zeta(2)=pi*pi/6.0d0             
          zeta(3)=1.20205690315959D0
          zeta(4)=1.08232323371114D0
             do n=1,1000
             do m=5,10
             zeta(m)=zeta(m)+1.0d0/dfloat(n)**m
             end do
             if(n.gt.995)then
             print '(8F15.10)',(zeta(m),m=1,10)
             end if
             end do
             do m=1,10
             sum(m,1)=0.0d0
        ...
        do n=1,mtot 
        do m=2,10   
        sum(m,1)=sum(m,1)+1.0/dfloat(n)**m
        end do
        if(40*(n/40).eq.n)print 10,n,(sum(m,1),m=2,10)               
        end do
        print '(6x,G15.10,"=pi^2/6")',pi*pi/6.0d0
        print*,' '
c convert outer sum to an integral
        print*,'converting residual sums to integrals'
        n2=2
        do l=2,400
             do m=1,10
             sum(m,2)=0.0d0
        ...
        do n=1,l-1
         do m=2,10
         sum(m,2)=sum(m,2)+1.0d0/dfloat(n)**m
         end do
        end do
c       print 10,l,(sum(m,2),m=2,10)
c       read*,dummy
c convert remaining sum to integral
c sum_{n=l}^[Inf] 1/n^m = int(l+1/2,inf] dn 1/n^m 
c             = 1/(1-m) 1/n^{m-1} [n=l+1/2,inf]
c             = ((l+1/2)**(1-m))/(m-1)
c         alp(2)=float(l)-0.5d0 ! should 0.5 = ?
c         alp(2)=pi*pi/6.0d0-sum(2,2)
c         alp(2)=1.0d0/alp(2)
c         alp(3)=2.0d0*(1.20205690315959D0-sum(3,2))
c         alp(3)=1.0d0/alp(3)**(1.0d0/2.0d0)
c         alp(4)=3.0d0*(1.08232323371114D0-sum(4,2))
c         alp(4)=1.0d0/alp(4)**(1.0D0/3.0d0)      
c         do m=5,10
c         alp(m)=alp(4)
c         end do
               do m=2,10
               alp(m)=(dfloat(m-1)*(zeta(m)-sum(m,2)))**(-1.0d0/(m-1))
               end do
          do m=2,10
c         sum(m,2)=sum(m,2)+(1.0d0/dfloat(m-1))/alp(m)**(m-1)
c correction = int_alp^inf  dn 1/n^m = int_0^(1/alp) 1/(u^2) du f(1/u)
c   u= 1/n          
c  du=-1/n^2 dn 
             onedinf=0.0d0
             onedalp=1.0d0/alp(m)
             nn=5
             call gaussp(onedinf,onedalp,nn,xn,wn)
c            print '(5F12.5)',(1.0/xn(i),i=1,5)
c            read*,dummy
             do i=1,nn
             wn(i)=(1.0d0/xn(i)**2)*wn(i)
             xn(i)=1.0d0/xn(i)
             sum(m,2)=sum(m,2)+ wn(i)*1.0d0/xn(i)**m
             end do
          end do
          if(n2*(l/n2).eq.l)then
            print 10,n,(sum(m,2),m=2,6),alp(2),alp(3),alp(8)
            n2=min(2*n2,400)
          end if
        end do
c now lets do it perfectly:
       sum=0.0d0 
       eps= 0.1 !(in applications we don't know what eps is)
       n2=1
       do n=1,400 
          do m=2,10
          sum(m,1)=sum(m,1)+1.0d0/(dfloat(n)-eps)**m
          sum(m,3)=sum(m,3)+1.0d0/(dfloat(n)    )**m
          sum(m,2)=sum(m,1)
          sum(m,4)=sum(m,3)
       alp(m)=(dfloat(m-1)*(zeta(m)-sum(m,3)))**(-1.0d0/dfloat(m-1))
             onedinf=0.0d0
             onedalp=1.0d0/alp(m)
             nn=5 
             call gaussp(onedinf,onedalp,nn,xn,wn)
             do i=1,nn
             sum(m,2)=sum(m,2)+ wn(i)*xn(i)**(m-2)
             sum(m,4)=sum(m,4)+ wn(i)*xn(i)**(m-2)
             end do
          end do
          if(n2*(n/n2).eq.n)then
                  n2=2*n2
          print 10,n,(sum(m,2)-sum(m,4),m=2,8),alp(8)                   
          end if
       end do
 10     format(i5,1x,15G15.10)
        end
C ***********************************************************
C
